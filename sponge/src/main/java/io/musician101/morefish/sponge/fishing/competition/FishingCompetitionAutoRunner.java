...
public final class FishingCompetitionAutoRunner {
    ...
    public final void enable() {
        ...
        timeCheckingTask = Sponge.asyncScheduler().submit(Task.builder().execute(new TimeChecker(this::tryOpenCompetition)).interval(30, TimeUnit.SECONDS).plugin(SpongeMoreFish.getInstance().getPluginContainer()).build());
    }
    ...
    private void tryOpenCompetition() {
        ...
        if (SpongeMoreFish.getInstance().getCompetition().isDisabled() && Sponge.server().onlinePlayers().size() >= requiredPlayers) {
