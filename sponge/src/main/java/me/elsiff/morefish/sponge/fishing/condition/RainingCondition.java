...
public record RainingCondition(boolean raining) implements FishCondition<SpongeFishingCompetition, Item, ServerPlayer> {

    public boolean check(@NotNull Item caught, @NotNull ServerPlayer fisher, @NotNull SpongeFishingCompetition fishingCompetition) {
        return caught.world().weather().type().equals(WeatherTypes.RAIN.get()) == raining;
    }
}
